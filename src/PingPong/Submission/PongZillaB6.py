...
def plan(current_time: Second, arm: Arm, time_bound: Second, seg: Seg, velocity: Vec) -> Control:
    # Check if time_bound lies in the future
    if time_bound < current_time:
        print("Time bound {} lies in the past with respect to current time {}.".format(time_bound, current_time))
        return Control.from_json([])
    
    lengths = [link.llen for link, _ in arm.comp]
    lengths.append(arm.bat.llen/2) # velocity is in the middle of seg

    endpoint = (seg.p + seg.q) / 2.0

    # Obtain goal angles
    goalangles = angles(solveInverse(ITERATIONS, arm, seg)[0])
    ...
    for i in range(len(lengths)):
        ...
        J[0][i] = x
        J[1][i] = y
    #J = get_jacobian(arm, goalangles, endpoint)

    # Obtain goal angular velocities
    ...
    for i in range(len(goalVel) - 1):
        ...
        accelerations.append(angular_acceleration(current_time, a, b))

    return Control.from_json(accelerations)

def angular_acceleration(t: Second, a: float, b: float) -> float:
    return 6.0 * a * t + 2.0 * b

def getJacobian(arm, goalangles, endpoint):
    # Define unit vector "k-hat" pointing along the Z axis.
    kUnitVec = np.array([[0,0,1]], dtype=np.float)

    endEffectorCoords = np.array([endpoint.x, endpoint.y, 0.0])
    numjoints = len(list(arm.comp))
    jacobian = np.zeros((3, numjoints), dtype=np.float)

    joint_locations = [np.array([p.x, p.y, 0.0]) for p in evaluateArm(arm)]

    # Utilize cross product to compute each row of the Jacobian matrix.
    for i in range(numjoints):
        currentJointCoords = joint_locations[i]
